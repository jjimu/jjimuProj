...
$(document).ready(function() {
    ...
    function updateNavigation() {
        ...
        $('.navigate-folder').off('click').on('click', function() {
            ...
            console.log("navigate-folder의 folderId : ", folderId);
    ...
    function navigateToFolder(folderId) {
        ...
        if (folder === null) {
            ...
            console.log('폴더 이동: Unknown Folder', '폴더ID:', folderId); // 폴더명 로그 출력
        } else {
            ...
            console.log('폴더 이동:', folder.name, '폴더ID:', folder.id, '자료실ID:', folder.drNo); // 폴더명 로그 출력
        }
            ...
            success: function(response) {
                if (response && response.length > 0) {
                    folder = {
                        ...
                        drNo: response[0].drNo // 자료실 ID 추가
                    };
                    console.log('폴더 정보 가져오기 성공:', response[0].fdNm, '폴더ID:', response[0].fdNo, '자료실ID:', response[0].drNo); // 폴더명 로그 출력
                }
            },
            error: function() {
    ...
    function permanentDeleteFile(fileId) {
        $.ajax({
            url: '/dataFolder/permanentDelete',
            type: 'POST',
            data: JSON.stringify({ id: fileId, type: 'file' }),
    ...
    function permanentDeleteSelected(selectedIds) {
        $.ajax({
            ...
            data: JSON.stringify({ items: selectedIds }),
            beforeSend: function(xhr) {
                xhr.setRequestHeader(csrfHeaderName, csrfToken);
            },
            success: function(response) {
                if (response.status === 'success') {
                    ...
                    updateStorageInfo();  // 삭제 후 스토리지 정보 업데이트
    ...
    function loadFileList() {
        var folderInfo = getCurrentFolderInfo(); // 현재 폴더 정보 가져오기
        console.log('리스트를 뿌리는 폴더명:', folderInfo.fdNm, '폴더ID:', folderInfo.fdNo, '자료실ID:', folderInfo.drNo); // 폴더명, 폴더ID, 자료실ID 출력
        ...
        $.ajax({
            ...
            success: function(response) { // 요청 성공 시 실행되는 함수
                    ...
                    files.forEach(function(file) {
                        if (file.fdNo == parentId) {
                            var listItem = 
                                ...
                                (file.dfFileSz / 1024).toFixed(2) + ' KB</small>' +
                                '<small class="data-list-item-time text-muted">' +
                ...
                // 필터 적용
                applyFilter(folderInfo.fdNo); // 현재 폴더 ID로 필터 적용
...
//                                         updateStorageInfo();  // 파일 삭제 후 스토리지 정보 업데이트
                ...
                $('.data-edit').off('click').on('click', function() {
                    ...
                    $('#nameEdit').on('shown.bs.modal', function() {
                        ...
                        // 엔터 키를 누르면 저장 버튼 클릭 이벤트 트리거
                        $('#modalNameEdit').off('keypress').on('keypress', function(e) {
                            if (e.which == 13) { // 엔터 키 코드 확인
                                $('#saveNameEdit').click(); // 저장 버튼 클릭
                            }
                        });
                    });
    ...
    function getCurrentFolderInfo() {
	    ...
	    var fdNm = navItems.length > 1 ? navItems.last().text() : ""; // 폴더명을 텍스트로 가져오기
	
	    return { drNo: drNo, fdNo: fdNo, fdNm: fdNm };
	}
    ...
    $('#uploadForm').on('submit', function(event) {
        ...
        console.log('현재 폴더명:', folderInfo.fdNm, '폴더ID:', folderInfo.fdNo, '자료실ID:', folderInfo.drNo); // 현재 폴더명, 폴더ID, 자료실ID 출력
    ...
    function updateStorageInfo() {
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#totalSize').text(totalSize + " (" + percentageUsed + ")");
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status === 'success') {
                            ...
                            updateStorageInfo();  // 삭제 후 스토리지 정보 업데이트 추가
                            loadFileList(); // 파일 목록 새로고침
                        } else {
                            Swal.fire({
                                title: '삭제 실패!',
    ...
    function applyFilter(folderId = null) {
        ...
        updateNavigation(); // 네비게이션 업데이트
        var lastNavFolderId = $('.data-nav span:last-child').data('id');
        console.log("lastNavFolderId", lastNavFolderId);

        $('.data-list-item').each(function() {
            var itemFolderUp = $(this).attr('folder-up');
            var itemFolderNo = $(this).attr('folder-no');

            // 최상위 자료실(예: DR001, DR002, DR003 등)을 리스트 표시
            if (folderId == null) {
            ...
            // 네비게이션의 첫 번째 항목(최상위 자료실)을 클릭했을 때 최상위 폴더와 파일을 표시
            else if (currentPath.length === 1 && ((itemFolderUp === "" || itemFolderUp === "null") || (itemFolderNo === "" || itemFolderNo === "null")) && $(this).data('target') == activeFilter) {
                $(this).show();
            }
            // 네비게이션의 마지막 값의 data-id 속성과 folder-up 속성이 같으면 해당 아이템을 표시
            else if (itemFolderUp == lastNavFolderId || itemFolderNo == lastNavFolderId) {
                $(this).show();
            }
            // 하위 폴더와 파일을 구분하여 표시
            else {
                if ((itemFolderUp == folderId || itemFolderNo == folderId) && $(this).data('target') == activeFilter) {
                    $(this).show();
                } else {
                    $(this).hide();
                }
            ...
            <div class="btn-compost-wrapper d-grid">
                <div class="data-aside-header">
				    ...
				    <div class="status-info">
				        <span id="usedSize">0 KB</span> / <span id="totalSize">2048 KB</span>
				    </div>
				</div>
